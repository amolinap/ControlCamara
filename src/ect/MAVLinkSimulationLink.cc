...
//        streampointer += bufferlength;
    ...
    if (rate1hzCounter == 1000 / rate / 1)
    {
        ...
        // Pack message and get size of encoded byte string
        messageSize = mavlink_msg_heartbeat_pack(systemId, componentId, &msg, mavType, MAV_AUTOPILOT_SLUGS);//MAV_AUTOPILOT_PIXHAWK);
        // Allocate buffer with packet data
        bufferlength = mavlink_msg_to_send_buffer(buffer, &msg);
        //add data into datastream
        memcpy(stream+streampointer,buffer, bufferlength);
...
bool MAVLinkSimulationLink::connect()
{
    ...
    start(LowPriority);
    MAVLinkSimulationMAV* mav1 = new MAVLinkSimulationMAV(this, 1, 19.1968, -96.1404); //47.376, 8.548);
    Q_UNUSED(mav1);
...
void MAVLinkSimulationLink::saveDataToFile(const QString &path)
{
    Q_UNUSED(path);    
}

void MAVLinkSimulationLink::stopWrite()
{
}
