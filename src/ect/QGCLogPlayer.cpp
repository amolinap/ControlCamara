...
#include "ui_QGCLogPlayer.h"

QGCLogPlayer::QGCLogPlayer(MAVLinkProtocol* mavlink, QWidget *parent) :
        QWidget(parent),
        lineCounter(0),
        totalLines(0),
        startTime(0),
...
QGCLogPlayer::~QGCLogPlayer()
{
    delete ui;
}

void QGCLogPlayer::play()
{
    if (logFile.isOpen())
    {
    ...
    else
    {
        ...
        msgBox.setText(tr("Archivo de registro no seleccionado"));
        msgBox.setInformativeText(tr("Selecciona un archivo de registro MAVLink antes de reproducirlo"));
...
bool QGCLogPlayer::reset(int packetIndex)
{
...
void QGCLogPlayer::selectLogFile()
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Especifique el nombre del registro MAVLink"), pathSaveData, tr("Archivos MAVLink o Binario (*.mavlink; *.bin);;"));
...
void QGCLogPlayer::setAccelerationFactorInt(int factor)
{
    float f = factor+1.0f;
    f -= 50.0f;
...
void QGCLogPlayer::loadLogFile(const QString& file)
{
    ...
    {
        //MainWindow::instance()->showCriticalMessage(tr("El archivo de registro no se puede leer"), tr("Asegurese que el archivo %1 es escribible o seleccione un archivo diferente").arg(file));
        logFile.setFileName("");
    }
    else
    {
        ...
        logFile.reset();
        startTime = 0;
        ui->logFileNameLabel->setText("../"+logFileInfo.completeBaseName()+".bin");//tr("%1").arg(logFileInfo.absolutePath().baseName()));
        ui->logFileNameLabel->setToolTip(file);
...
void QGCLogPlayer::jumpToSliderVal(int slidervalue)
{
    loopTimer.stop();
...
void QGCLogPlayer::changeEvent(QEvent *e)
{
...
void QGCLogPlayer::changePATH(const QString &path)
{
    this->pathSaveData = path;
}
